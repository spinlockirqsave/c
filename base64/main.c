/*
 * @file    main.c
 * @date    2 Mar 2016 1.13 PM
 * @author  Piotr Gregor <piotrek.gregor gmail com>
 * @brief   Binary to text encode/decode in base64 format.
 * @details Reuses Brad Conte's (brad AT bradconte.com)
 *          code.
 */
...
size_t
base64_encode(const BYTE in[], BYTE out[], size_t len, int newline_flag)
{
...
size_t
base64_decode(const BYTE in[], BYTE out[], size_t len)
{
	BYTE ch;
 ...
 */
bool
encodebase64(const unsigned char *pBinaryData, int iDataLen,
        char *pszOutput, int *piOutputLen)
{
    int len;

    if (pBinaryData == NULL || pszOutput == NULL
            || piOutputLen == NULL || iDataLen < 1)
        return 1;
    len = base64_encode(pBinaryData, pszOutput, iDataLen, 1);
    *piOutputLen = len;
...
base64_test2(void)
{
	...
	for (idx = 0; idx < 3; idx++) {
		encodebase64(text[idx], strlen(text[idx]), buf, &buf_len);
		pass = pass && ((buf_len == strlen(code[idx])));
		pass = pass && !strcmp(code[idx], buf);
		memset(buf, 0, sizeof(buf));
	}
...
int main()
{
	printf("Base64 tests 1: %s\n", base64_test1() ? "PASSED" : "FAILED");
	printf("Base64 tests 2: %s\n", base64_test2() ? "PASSED" : "FAILED");
